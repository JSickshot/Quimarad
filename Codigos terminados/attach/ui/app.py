...
class App(tk.Tk):
    ...
    def _build_copy_page(self):
        ...
        ttk.Frame(page).pack(fill="both", expand=True, padx=16, pady=(4, 16))
        ttk.Label(page, text="* Ejecuta como Administrador ", foreground="#666").pack(anchor="w", padx=20)
    ...
    def _scan_mdfs(self, root: str) -> Dict[str, Tuple[str, Optional[str]]]:
        ...
        for dirpath, _, files in os.walk(root):
            for fn in files:
                ...
                ldf = next((os.path.normpath(c) for c in cand if os.path.exists(c)), None)
                out[name] = (mdf, ldf)
        return out
    ...
    def _attach_one(self, conn, name: str, mdf: str, ldf: Optional[str], force_rebuild_log: bool = False):
        ...
        if force_rebuild_log:
            tsql = f"CREATE DATABASE {name_q} ON (FILENAME = N'{mdf_q}') FOR ATTACH_REBUILD_LOG;"
            conn.cursor().execute(tsql)
            return

        raise last_err if last_err else RuntimeError("No se pudo adjuntar la base.")
        ...
        try:
            with sql_conn(inst, trusted=False, user=self.a_user.get().strip(), password=self.a_pwd.get()) as conn:
                cur = conn.cursor(); cur.execute(
                    "SELECT @@SERVERNAME, SYSTEM_USER, DB_NAME(), CONVERT(nvarchar(4000), SERVERPROPERTY('InstanceDefaultDataPath'))"
                )
                s, u, db, datapath = cur.fetchone()
            self._alog(f"[OK] {s} — Usuario: {u}")
            self._alog(f"Ruta de DATA de la instancia: {datapath}")
    ...
    def _run_attach_flow(self):
        ...
        def worker():
            ...
            try:
                ...
                if copied:
                    for _, d in copied:
                        ...
                        base, ext = os.path.splitext(os.path.basename(d))
                        if ext.lower() == ".mdf":
            ...
            try:
                with sql_conn(inst, trusted=False, user=self.a_user.get().strip(), password=self.a_pwd.get()) as conn:
                    self._alog("Adjuntando CATÁLOGOS")
                    catalogs = [
                        "DB_Directory",
                        "predeterminada",
                        "generalessql",
                    ...
                    self._alog("Adjuntando EMPRESAS")
                    idx = self._scan_mdfs(data_root)
                    intent: Set[str] = {n for n in copied_names if n not in self._CATALOG_NAMES}

                    if not intent:
                        try:
                            cur = conn.cursor()
                            cur.execute("""
                                IF DB_ID('DB_Directory') IS NOT NULL
                                   AND OBJECT_ID('DB_Directory..Empresas') IS NOT NULL
                                SELECT Nombre FROM DB_Directory..Empresas
                            """)
                            rows = cur.fetchall()
                            if rows:
                                intent = {str(r[0]).strip() for r in rows if r and r[0]}
                        except Exception:
                            pass
                    ...
                    if not intent:
                        intent = {
                            n for n in idx.keys()
                            if n not in self._CATALOG_NAMES and n not in self._SYSTEM_DB_NAMES
                        }

                    def fam(n: str) -> str:
                        ln = n.lower()
                        if "nom" in ln: return "Nóminas"
                        if "com" in ln or "comer" in ln: return "Comercial"
                        return "Contabilidad"

                    def _looks_add(n: str) -> bool:
                        ln = n.lower()
                        return ("add" in ln) or ln.startswith("document_") or ln.startswith("other_")

                    intent_emp = {n for n in intent if not _looks_add(n) and n in idx}
                    orden = ["Contabilidad", "Nóminas", "Comercial"]
                    adj_ok, adj_fail = set(), set()

                    for grupo in orden:
                        self._alog(f"    {grupo}")
                        for n in sorted(intent_emp):
                            if fam(n) != grupo:
                                continue
                            mdf, ldf = idx[n]
                            try:
                                if self._db_exists(conn, n):
                                    self._alog(f"      [skip] {n}: ya existe")
                                    continue
                                if n in self._SYSTEM_DB_NAMES or n in self._CATALOG_NAMES:
                                    self._alog(f"      [skip] {n}: excluida (sistema/catálogo)")
                                    continue
                                force_rb = (grupo == "Nóminas") or n.lower().startswith("nom")
                                self._alog(f" {n}  {mdf}  {('['+ldf+']' if ldf else '(sin LDF)')}  force_rebuild={force_rb}")
                                self._attach_one(conn, n, mdf, ldf, force_rebuild_log=force_rb)
                                self._alog(f"      [OK]  {n}")
                                adj_ok.add(n)
                            except Exception as ex:
                    ...
                    self._alog(" Adjuntando ADD ")
                    intent_add = {n for n in intent if n in idx and _looks_add(n)}
                    if not intent_add:
                        intent_add = {n for n in idx if _looks_add(n)}
                    intent_add = {n for n in intent_add if n not in self._CATALOG_NAMES and n not in self._SYSTEM_DB_NAMES}

                    for n in sorted(intent_add):
                        mdf, ldf = idx[n]
                        try:
                            if self._db_exists(conn, n):
                                self._alog(f"      [skip] {n}: ya existe")
                            ...
                            self._attach_one(conn, n, mdf, ldf, force_rebuild_log=False)
                            self._alog(f"      [OK]  {n}")
                            adj_ok.add(n)
                        except Exception as ex:
                            self._alog(f"      [ERR] {n}: {ex}")
                ...
                self._alog("Migración (attach) finalizada.")
